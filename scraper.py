...
def download_card_image(image_url, set_name, set_code, card_number):
    try:
        # Create a safe directory name from the set name
        safe_set_name = set_name.lower().replace(" ", "-").replace("'", "").replace(",", "")
        ...
        # Download the image
        print(f"Downloading image for {set_name} - {card_number} from {image_url}")
        ...
        # This will be used in HTML to reference the image
        return f"cards/{safe_set_name}/{filename}"
    except Exception as e:
        print(f"Error downloading image for {set_name} - {card_number}: {e}")
        return None
...
def setup_database():
    ...
    # Add column for energy_type if it doesn't exist
    if 'energy_type' not in columns:
        try:
            cursor.execute('ALTER TABLE cards ADD COLUMN energy_type TEXT')
            print("Added energy_type column to cards table")
        except sqlite3.OperationalError as e:
            print(f"Note: {e}")
    
    # Add column for rarity if it doesn't exist
    ...
    # Add column for pack if it doesn't exist
    if 'pack' not in columns:
        try:
            cursor.execute('ALTER TABLE cards ADD COLUMN pack TEXT')
            print("Added pack column to cards table")
        except sqlite3.OperationalError as e:
            print(f"Note: {e}")
    
    # Add column for local_image_path if it doesn't exist
    if 'local_image_path' not in columns:
        try:
            cursor.execute('ALTER TABLE cards ADD COLUMN local_image_path TEXT')
            print("Added local_image_path column to cards table")
        except sqlite3.OperationalError as e:
            print(f"Note: {e}")
...
def save_to_csv(card_data, write_header=False):
    ...
    with open("pokemon_cards.csv", mode, newline="") as f:
        writer = csv.writer(f)
        if write_header:
            writer.writerow(["Name", "Energy Type", "Set Name", "Set Code", "Card Number", "Card Type", "HP", "Attacks", "Weakness", "Retreat Cost", "Illustrator", "Flavor Text", "Image URL", "Rarity", "Pack", "Local Image Path"])
        elif card_data:
...
def save_to_database(card_data):
    ...
    if 'energy_type' in columns and 'rarity' in columns and 'pack' in columns and 'local_image_path' in columns:
        # All columns exist
        cursor.execute('''
            INSERT INTO cards (name, energy_type, set_name, set_code, card_number, card_type, hp, attacks, weakness, retreat_cost, illustrator, flavor_text, image_url, rarity, pack, local_image_path)
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
        ''', card_data)
    elif 'energy_type' in columns and 'rarity' in columns and 'pack' in columns:
        # No local_image_path column
        cursor.execute('''
            INSERT INTO cards (name, energy_type, set_name, set_code, card_number, card_type, hp, attacks, weakness, retreat_cost, illustrator, flavor_text, image_url, rarity, pack)
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
        ''', card_data[:15])
    else:
        # Minimal columns
        name, energy_type, *rest = card_data  # Unpack to separate energy_type
        cursor.execute('''
            INSERT INTO cards (name, set_name, set_code, card_number, card_type, hp, attacks, weakness, retreat_cost, illustrator, flavor_text, image_url)
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
        ''', (name, *rest[1:13]))
...
def scrape_card_details(set_code, card_number):
    ...
    try:
        ...
        local_image_path = download_card_image(image_url, set_name, set_code, card_number)
        ...
        if "Pokémon" in card_type:
            ...
            for attack in attack_elements:
                ...
                attack_cost = [cost.text.strip() for cost in energy_costs] if energy_costs else []
